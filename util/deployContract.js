...
async function init(contractName){
  try {
    ...
    result = await exec(`
    cleos -u ${env.endpoints[env.network].rpc} set code ${accounts[env.network][contractName]} ${path.join(__dirname,`../_compiled_contracts/${contractName}/jungle/${contractName}.wasm`)}
    `)
    if (result) console.log(result.stdout)
